...
-- Useful if gg=G breaks identation due to false positives.
function ToTabs()
	...
	-- If identation was inconsistent we can match for spaces
	-- in the beginning of the line. We can either notify user and undo changes
	-- or even replace remaining spaces with 1 tab.
	--vim.cmd([[silent %s/^\(\t*\) \+/\1\t]])
	vim.fn.winrestview(curpos)
end
-- Convert leading tabs to spaces based on tabstop
function ToSpaces()
	local tabstop     = vim.bo.tabstop
	local space_pat   = string.rep(' ', tabstop)  -- make a pattern of tabstop spaces
	local pattern     = [[^\(\%(]] .. space_pat .. [[\)*\)\t]]
	local replacement = [[\1]] .. space_pat
	local count       = 0
