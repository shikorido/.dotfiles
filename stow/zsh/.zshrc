# Make sure hook function arrays are always defined.
() {
    hooktypes=(chpwd precmd preexec periodic zshaddhistory zshexit zsh_directory_name)
    typeset -ga ${^hooktypes}_functions
}

[[ -f ~/.zsh_env_persistent ]] && . ~/.zsh_env_persistent

if [[ $_MSYS2_ && $TERM_PROGRAM = WezTerm ]]; then
    # Keep correct state for re-sourcing (aka reloading).
    unset __WEZTERM_CHPWD_INIT __WEZTERM_CHPWD_HOOK
fi
...
if [[ -z $TMPDIR ]]; then
    if [[ $_TERMUX_ ]]; then
        ...
        # ownership (might be root) and existence checks.
...
# For fzf-tab in termux, but since I try to unify the configs,
# the TMPDIR var has also been defined in all linux-based environments.
export TMPPREFIX=$TMPDIR/zsh

# Common variables that might not be defined.
...
if [[ $_MSYS2_ ]]; then
    ...
    fpath=( /$MSYSTEM/share/zsh/site-functions $fpath )
fi
...
plugins=(
    ...
    #
    # It should be fast but why does it slow down printing/pasting by 10 times..? (In msys2 at least).
...
# Sidenote: `.` means the same as `source` but the latter is not defined by POSIX.
