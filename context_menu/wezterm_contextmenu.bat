...
@for /f %%f in ('dir /b /a:-d-l') do (
    @if [%%~nxf] == [wezterm-gui.exe] (
        echo wezterm-gui.exe was found in the current script directory.
        echo.
...
set REG_KEY=HKLM\SOFTWARE\Classes\Directory\background\shell\045_OpenInWezTerm
...
reg add "%REG_KEY%" /v MUIVerb /d "WezTerm"
...
REM Path to registry key
set REG_KEY=HKLM\SOFTWARE\Classes\Directory\shell\045_OpenInWezTerm
REM Path to file in filesystem
set FSP_LOC=%~1
echo REG_KEY is %REG_KEY%
echo FSP_LOC is %FSP_LOC%
...
if [%~1] == [User] (
    reg add "%REG_KEY%\shell\%count%%~1\command" /ve /d "\"%FSP_LOC%\" start --cwd \"%%V.\""
) else (
    reg add "%REG_KEY%\shell\%count%%~1\command" /ve /d "powershell -Command Start-Process \"\\\"%FSP_LOC%\\\"\" -ArgumentList start,'--cwd',\"\\\"`\\\"%%V.`\\\"\\\"\" -Verb RunAs"
)
@echo off
exit /b 0
...
if [%~1] == [User] (
    reg add "%REG_KEY%\shell\%count%%~1\command" /ve /d "\"%FSP_LOC%\" start --cwd \"%%1\""
) else (
    reg add "%REG_KEY%\shell\%count%%~1\command" /ve /d "powershell -Command Start-Process \"\\\"%FSP_LOC%\\\"\" -ArgumentList start,'--cwd',\"`\\\"\\\"%%1`\\\"\\\"\" -Verb RunAs"
)
@echo off
exit /b 0
