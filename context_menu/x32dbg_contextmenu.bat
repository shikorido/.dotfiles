...
@for /f %%f in ('dir /b /a:-d-l') do (
    @if [%%~nxf] == [x32dbg-unsigned.exe] (
        echo x32dbg-unsigned.exe was found in the current script directory.
        echo.
...
@for %%x in (dllfile exefile) do (
    ...
    REM set REG_KEY=HKLM\SOFTWARE\Classes\%%x\shell\094_x32dbg
    REM Avoiding EnableDelayedExpansion by passing REG_KEY directly
    call :loop_call "HKLM\SOFTWARE\Classes\%%x\shell\094_x32dbg"
)
