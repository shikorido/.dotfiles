...
set count=01
@for %%x in (MD5 SHA1 SHA256 SHA384 SHA512) do (
    call :loop_call "%%x"
    set /a count+=1
    if !count! lss 10 (
        set count=0!count!
...
reg add "%REG_KEY%\shell\%count%%~1\command" /ve /d "powershell -windowstyle hidden -command \"^(get-filehash -algorithm %~1 -path '%%1'^).hash.tolower().tostring()^|set-clipboard\""
@echo off
exit /b 0
