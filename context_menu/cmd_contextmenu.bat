...
REM For adding windows' stock cmd change current dir to System32
REM cd /d "%SYSTEMROOT%\System32"

REM @for /f %%f in ('dir /b /a:-d-l') do (
REM     @if [%%~nxf] == [cmd.exe] (
REM         echo cmd.exe was found in the current script directory.
REM         echo.
...
for /f "delims=" %%F in ('where cmd.exe') do (
    set cmd_loc=%%F
    goto :got_cmd
)
...
set REG_KEY=HKLM\SOFTWARE\Classes\Directory\background\shell\025_OpenInCmd
...
reg add "%REG_KEY%" /v MUIVerb /d "cmd"
...
REM Path to registry key
set REG_KEY=HKLM\SOFTWARE\Classes\Directory\shell\025_OpenInCmd
REM Path to file in filesystem
set FSP_LOC=%~1
echo REG_KEY is %REG_KEY%
echo FSP_LOC is %FSP_LOC%
...
if [%~1] == [User] (
    reg add "%REG_KEY%\shell\%count%%~1\command" /ve /d "\"%FSP_LOC%\" /s /k pushd \"%%V.\""
) else (
    reg add "%REG_KEY%\shell\%count%%~1\command" /ve /d "powershell -Command Start-Process \"\\\"%FSP_LOC%\\\"\" -ArgumentList '/s','/k',pushd,\"\\\"%%V.\\\"\" -Verb RunAs"
)
@echo off
exit /b 0
...
if [%~1] == [User] (
    reg add "%REG_KEY%\shell\%count%%~1\command" /ve /d "\"%FSP_LOC%\" /s /k pushd \"%%1\""
) else (
    reg add "%REG_KEY%\shell\%count%%~1\command" /ve /d "powershell -Command Start-Process \"\\\"%FSP_LOC%\\\"\" -ArgumentList '/s','/k',pushd,\"\\\"%%1\\\"\" -Verb RunAs"
)
@echo off
exit /b 0
