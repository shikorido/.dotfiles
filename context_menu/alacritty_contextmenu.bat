...
@for /f %%f in ('dir /b /a:-d-l') do (
    @if [%%~nxf] == [Alacritty.exe] (
        echo Alacritty.exe was found in the current script directory.
        echo.
...
set REG_KEY=HKLM\SOFTWARE\Classes\Directory\background\shell\055_OpenInAlacritty
...
reg add "%REG_KEY%" /v MUIVerb /d "Alacritty"
...
REM Path to registry key
set REG_KEY=HKLM\SOFTWARE\Classes\Directory\shell\055_OpenInAlacritty
REM Path to file in filesystem
set FSP_LOC=%~1
echo REG_KEY is %REG_KEY%
echo FSP_LOC is %FSP_LOC%
...
if [%~1] == [User] (
    reg add "%REG_KEY%\shell\%count%%~1\command" /ve /d "\"%FSP_LOC%\" --working-directory \"%%V.\""
) else (
    reg add "%REG_KEY%\shell\%count%%~1\command" /ve /d "powershell -Command Start-Process \"\\\"%FSP_LOC%\\\"\" -ArgumentList '--working-directory',\"\\\"`\\\"%%V.`\\\"\\\"\" -Verb RunAs"
)
@echo off
exit /b 0
...
if [%~1] == [User] (
    reg add "%REG_KEY%\shell\%count%%~1\command" /ve /d "\"%FSP_LOC%\" --working-directory \"%%1\""
) else (
    reg add "%REG_KEY%\shell\%count%%~1\command" /ve /d "powershell -Command Start-Process \"\\\"%FSP_LOC%\\\"\" -ArgumentList '--working-directory',\"\\\"`\\\"%%1`\\\"\\\"\" -Verb RunAs"
)
@echo off
exit /b 0
