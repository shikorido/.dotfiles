...
@for /f %%f in ('dir /b /a:-d-l') do (
    @if [%%~nxf] == [OllyDBG.exe] (
        echo OllyDBG.exe was found in the current script directory.
        echo.
...
@for %%x in (dllfile exefile) do (
    ...
    REM set REG_KEY=HKLM\SOFTWARE\Classes\%%x\shell\086_OllyDBG2.01
    REM Avoiding EnableDelayedExpansion by passing REG_KEY directly
    call :loop_call "HKLM\SOFTWARE\Classes\%%x\shell\086_OllyDBG2.01"
)
