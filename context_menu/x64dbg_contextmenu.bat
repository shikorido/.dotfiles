...
@for /f %%f in ('dir /b /a:-d-l') do (
    @if [%%~nxf] == [x64dbg-unsigned.exe] (
        echo x64dbg-unsigned.exe was found in the current script directory.
        echo.
        echo Adding it to the context menu for dll and exe files.
        echo.
...
@for %%x in (dllfile exefile) do (
    REM Path to registry key
    REM set REG_KEY=HKLM\SOFTWARE\Classes\%%x\shell\095_x64dbg
    REM Avoiding EnableDelayedExpansion by passing REG_KEY directly
    call :loop_call "HKLM\SOFTWARE\Classes\%%x\shell\095_x64dbg"
)
...
@echo on
reg delete "%~1" /f
reg add "%~1" /ve /d "x64dbg"
reg add "%~1" /v Icon /d "%FSP_LOC%"
reg add "%~1\command" /ve /d "powershell -Command Start-Process \"\\\"%FSP_LOC%\\\"\" -ArgumentList \"\\\"`\\\"%%1`\\\"\\\"\" -Verb RunAs"
@echo off
exit /b 0
